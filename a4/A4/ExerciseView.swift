    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            HStack(alignment: .top, spacing: 16) {
                // Print statement to debug the URL
                let gifUrls = "http://34.59.215.239/api/gifs/\(exercise.id)/"
                    
                    // Use a separate method to print this to avoid view hierarchy issues
              

                // GIF with a subtle border for white backgrounds
                AnimatedImage(url: URL(string: gifUrls))
                    ...
                    .onAppear {
                        print("AnimatedImage appeared with URL: \(gifUrls)")
                    }
